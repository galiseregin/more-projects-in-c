...
boolean checkObjectIsValid(char board[][SIZE], int i, int j, int isWhiteTurn);
...
/********************************************************************************************************************
* Function name: checkObjectIsValid
* Input: char board[][SIZE](valid values: 2d array of any chess pieces)
...
boolean tryAndMoveCapture(char board[][SIZE], char pgn[], int isWhiteTurn)
{
	...
	if (checkObjectIsValid(board, destRow, destCol, isWhiteTurn))
		return FALSE;
...
void markDangerousSpots(char board[][SIZE], char dangerousCellsBoard[SIZE][SIZE], int kingRow, int kingCol, int oppositeColor)
{
	...
	for (i = 0; i < SIZE; i++)
	{
		for (j = 0; j < SIZE; j++)
		{
			...
			if (checkObjectIsValid(board, i, j, oppositeColor))
			{
				if (piece == PAWN)
				{
